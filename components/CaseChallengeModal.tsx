...
import { Target, X, Send, Award, Sparkles, AlertCircle, RefreshCw, CheckCircle } from 'lucide-react';
...
export const CaseChallengeModal: React.FC<CaseChallengeModalProps> = ({ topic, onClose }) => {
  ...
  const submitSolution = async () => {
    if (!userSolution.trim() || !challengePrompt) return;
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="text-xs font-semibold text-pink-400 uppercase tracking-wider">
                  Simulação de Caso Prático de Growth
                </span>
            ...
            {loadingChallenge && (
              <div className="py-16 flex flex-col items-center justify-center space-y-3">
                ...
                <p className="text-xs font-medium text-slate-300">
                  Criando cenário de dilema de produto realista para {topic.title}...
                </p>
              </div>
            )}
              ...
              <div className="space-y-6">
                ...
                <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-3">
                  <div className="flex items-center justify-between text-xs text-pink-400 font-bold">
                    ...
                    <button
                      onClick={() => generateChallenge(topic)}
                      className="text-slate-400 hover:text-slate-200 flex items-center space-x-1 font-normal text-[11px]"
                    >
                ...
                {!evaluation && (
                  <div className="space-y-3">
                    <label className="block text-xs font-bold text-slate-300">
                      Sua Resposta como Growth PM (Diagnóstico, Métricas e Hipóteses de Solução):
                    </label>
                    <textarea
                      rows={5}
                      value={userSolution}
                    ...
                    >
                      {loadingEvaluation ? (
                        <>
                          ...
                          <span>Avaliando com visão de VP de Growth...</span>
                        </>
                      ) : (
                        <>
                          <Send className="w-4 h-4" />
                          <span>Enviar Solução para Feedback do VP de Growth</span>
                        </>
                      )}
                    </button>
                  </div>
                ...
                {evaluation && (
                  <div className="space-y-4 bg-slate-950/80 p-5 rounded-2xl border border-pink-500/30">
                    <div className="flex items-center space-x-2 text-pink-400 font-bold text-sm">
                      ...
                      <span>FEEDBACK E AVALIAÇÃO DO VP DE GROWTH</span>
                    </div>
