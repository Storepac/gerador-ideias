...
export const ProductContextModal: React.FC<ProductContextModalProps> = ({ isOpen, onClose, productContext, onSaveProductContext }) => {
  const [contextInput, setContextInput] = useState(productContext);

  useEffect(() => {
    if (isOpen) setContextInput(productContext);
      ...
      <div className="w-full max-w-lg rounded-3xl border border-blue-500/20 bg-[#0b1728] p-6 shadow-2xl shadow-blue-950/30 sm:p-8">
        <div className="flex items-start justify-between gap-4 border-b border-white/10 pb-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-blue-500/20 bg-blue-500/10 text-blue-300"><BookOpen className="h-5 w-5" aria-hidden="true" /></div>
            <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-300">Personalização</p><h2 id="product-context-title" className="mt-1 text-xl font-bold text-white">Contexto do produto</h2></div>
          </div>
          <button type="button" onClick={onClose} aria-label="Fechar contexto do produto" className="rounded-xl p-2 text-slate-400 transition hover:bg-white/5 hover:text-white"><X className="h-5 w-5" aria-hidden="true" /></button>
        </div>
        ...
        <form onSubmit={handleSave} className="mt-6 space-y-5">
            ...
            <div className="space-y-2">
              {presets.map((preset) => (
                <button key={preset} type="button" onClick={() => setContextInput(preset)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3 text-left text-sm text-slate-300 transition hover:border-blue-500/20 hover:bg-blue-500/5 hover:text-white">
                  <span>{preset}</span><Sparkles className="h-4 w-4 shrink-0 text-blue-300" aria-hidden="true" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 border-t border-white/10 pt-5">
            ...
            <button type="submit" className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-500"><Check className="h-4 w-4" aria-hidden="true" />Salvar contexto</button>
          </div>
        </form>
      </div>
    </div>
